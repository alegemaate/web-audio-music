...
const WaveformDraw: React.FC = () => {
  ...
  const [synth, setSynth] = React.useState<AdditiveSynth | null>(null);
  React.useEffect(() => {
    if (!context || !gain) {
      return undefined;
    }
    const s = new AdditiveSynth(context, gain);
    setSynth(s);
    return () => {
      s.destroy();
      setSynth(null);
  ...
  return (
    <Layout>
      ...
      <Typography variant="body1" sx={{ mb: 2 }}>
        Sketch the real and imaginary harmonic components of a custom
        periodic wave, then watch the resulting waveform play back on the
        oscilloscope.
      </Typography>
      <Card>
            ...
            onChange={(_event, value) => {
              if (typeof value === "number") {
                if (points.real.length > value) {
                  ...
                  points.real.forEach((val, i) => {
                    real[i] = val;
                  });
                  points.imag.forEach((val, i) => {
                    imag[i] = val;
                  });
                  setPoints({ real, imag });
                }
