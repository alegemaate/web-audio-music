...
import { type FmPreset, FmSynth, FM_INSTRUMENTS } from "../components/FmSynth";
...
const FmSynthController: React.FC = () => {
  ...
  const [fmSynth, _fmSynth] = React.useState(() =>
  ...
  const copyPreset = () => {
    navigator.clipboard
      .writeText(btoa(JSON.stringify(preset)))
      .then(() => {
        setCopied(true);
        setTimeout(() => {
          setCopied(false);
  ...
  const loadPreset = () => {
    try {
      const parsed: unknown = JSON.parse(atob(presetText));
  ...
  return (
    <Layout>
      <Typography variant="h1">FM Synth</Typography>
      <Typography variant="body1" style={{ marginBottom: 16 }}>
        Control the FM Synth with a preset or create your own!
      </Typography>
          ...
          <Card>
            ...
            <CardActions>
              ...
              >
                ...
                {FM_INSTRUMENTS.map((instrument, index) => (
                  <MenuItem value={index} key={instrument.name}>
                    {instrument.name ?? `Preset ${index}`}
                  </MenuItem>
                ))}
              </Select>
            </CardActions>
...
export const Head = (): JSX.Element => (
  <Seo title="FM Synth" description="Web Audio FM Synthesizer" />
);
